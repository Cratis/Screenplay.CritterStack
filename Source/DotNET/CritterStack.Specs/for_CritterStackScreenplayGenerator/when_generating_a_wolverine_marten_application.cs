...
public class when_generating_a_wolverine_marten_application : given.a_wolverine_marten_application
{
    ...
    [Fact] void should_activate_the_handler_interface_convention() => _result.Graph.Artifacts.Any(_ => _.Key.Kind == ArtifactKind.Message && _.Key.Subject.Value.EndsWith("/IncidentService.InterfaceTrigger", StringComparison.Ordinal)).ShouldBeTrue();
    [Fact] void should_not_activate_generated_handlers() => _result.Graph.Artifacts.Any(_ => _.Key.Kind == ArtifactKind.Message && _.Key.Subject.Value.EndsWith("/IncidentService.GeneratedTrigger", StringComparison.Ordinal)).ShouldBeFalse();
