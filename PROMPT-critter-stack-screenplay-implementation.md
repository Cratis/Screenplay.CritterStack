...
- Critter Stack PR #23 is merged and released as `v0.5.0`. Exact direct Wolverine bus calls retain send, publish, request/reply, scheduling, delivery-option scheduling, and topic broadcast as distinct messaging relationships, never events.
- Critter Stack PR #24 is merged and released as `v0.6.0`. Direct-bus-only handlers become Reaction artifacts with Message triggers and Handles/Publishes relationships rather than disappearing or becoming commands/events. Verification passed 92 specs, zero-warning/error builds, sentinel packing, all PR checks, and all six canonical fixtures; real HelpDesk source proves the publish-only automation.
- Verified current packages are attached to GitHub releases and installed in `~/.nuget/cratis-local` with Generation 0.1.0. Local scratch consumers restore and execute without workflow changes.
- `VOGEN_CONCEPT_DISCOVERY_RESEARCH.md` defines neutral concept facts/lowering, reusable authored-source concept discovery, and a separate Vogen interpreter. Delivery is tracked in Screenplay.Generation #6/#7 and Critter Stack #25.
...
- Current Critter Stack `v0.6.0` publish run `32568782112` passed release/restore/build/pack and failed only at NuGet OIDC login because the owner-scoped trusted-publishing policy is still absent.
...
4. Implement neutral concept facts, subject-aware references, deterministic resolution, and top-level concept lowering in Screenplay.Generation #6.
