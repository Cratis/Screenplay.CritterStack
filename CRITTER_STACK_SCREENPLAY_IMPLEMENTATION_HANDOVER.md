...
- The current Marten issue #3 continuation discovers configured, attributed, conventional, and inherited document identities; unresolved or shadowed configured members produce `MARTEN0005` instead of falling back to a guess. Exact invocation-selected `IQuerySession`/`IBatchedQuery` compiled-query interfaces link document reads and public plan parameters only to proven Wolverine HTTP query entry points; unresolved nested executable flow produces `MARTEN0006`.
