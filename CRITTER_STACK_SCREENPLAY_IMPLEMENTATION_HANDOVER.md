...
- Remaining distribution blocker: Einar must configure two owner-scoped nuget.org trusted-publishing policies. Critter Stack `v0.3.0` run `32540542422` built and packed successfully, then failed only at NuGet OIDC login because no matching policy exists. CLI `v2.12.0` temporarily restores the verified `v0.1.0` adapter packages from GitHub release assets during CI/release builds; remove that bootstrap and update the bundled provider after normal nuget.org publication.
