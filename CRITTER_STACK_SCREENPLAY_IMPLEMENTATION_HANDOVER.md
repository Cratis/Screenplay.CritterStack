...
- Remaining distribution blocker: Einar must configure two owner-scoped nuget.org trusted-publishing policies. Critter Stack `v0.4.0` run `32565807453` passed release/restore/build/pack and failed only at NuGet OIDC login because no matching policy exists. CLI `v2.12.0` temporarily restores the verified `v0.1.0` adapter packages from GitHub release assets during CI/release builds; rerun the baseline, v0.3.0, and current v0.4.0 publishes after policy setup, then remove the bootstrap and update the bundled provider.
