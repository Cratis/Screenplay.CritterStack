...
- `CRITTER_STACK_PATTERN_DISCOVERY_RESEARCH.md` defines the source-evidence and resolution rules for State Change, State View, Automation, and Translation patterns across Marten projections/subscriptions and Wolverine handlers/consequences. `COMPATIBILITY.md` records exact canonical package sets, source baselines, support tiers, and the future CLI-owned package-provenance plan.
- Remaining distribution blocker: Einar must configure two owner-scoped nuget.org trusted-publishing policies. CLI `v2.11.0` temporarily restores the verified `v0.1.0` adapter packages from GitHub release assets during CI/release builds; remove that bootstrap after normal nuget.org publication.
