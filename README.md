...
- Full authored Wolverine saga discovery for public concrete closed `Wolverine.Saga` state, every current role spelling and `Async` twin, signature-stable handlers, exact/conventional message correlation, cascades, timeouts, direct bus calls, and exact `MarkCompleted()` evidence. `WOLVERINE0016`/`WOLVERINE0017` make workflow persistence and runtime-envelope correlation loss explicit without inventing lifecycle document operations or transport topology.
