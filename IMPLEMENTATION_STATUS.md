...
- Full authored Wolverine saga discovery admits only public concrete closed exact `Wolverine.Saga` derivatives active under source-resolved discovery/include/ignore policy. It emits unplaced Saga state, signature-stable Handler, Message, and role-discriminated Handles evidence; follows exact current correlation precedence; preserves ordinary cascades, timeout loss, direct bus and document operations; and reports `WOLVERINE0016`/`WOLVERINE0017` without inventing lifecycle persistence or transport topology.
...
4. Continue Marten #3—richer daemon/subscription consequences, arbitrary expression reconstruction, and non-Wolverine entry points—or Wolverine #4—compound middleware, full sagas/richer DCB, transport topology, subscriptions/forwarding, and projection side effects.
...
- Current/legacy return slots, direct-stream cascades, direct bus delivery, pure bus automations, return-only/`OutgoingMessages` automations, exact source-bound handler discovery activation, bounded DCB source evidence, and full authored saga state/role/correlation/lifecycle-loss evidence are classified. Runtime/custom predicate discovery, external assembly and handler-module scanning, saga persistence-provider/final-state semantics, richer transport topology, arbitrary DCB query/data flow, runtime descriptors, `FromConditions`, cross-boundary routing, subscriptions/forwarding, and projection side effects remain.
